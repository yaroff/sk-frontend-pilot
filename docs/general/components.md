...
One of most compelling arguments for self-contained components
architecture is ability to easily reuse each component in other projects.
Since all files kept in same folder, this should be a breeze.
...
component to its own git repository and npm package since keeping version
histories separate makes a lot of sense.

You're not finished with component, but would like to continue working on it
in parallel alongside your main project.

## How?

Since all files are kept in same place, its simply a matter of moving
folder to its own directory, setting up `package.json` for your new
...
Linking packages won't save package as a dependency in your main project
...
This boilerplate uses webpack DllPlugin which optimizes build times during
...
You can specify that you don't want this package to be included in dll by
adding following to your `package.json`.
...
to keep things separate while still working on them at same time.
...
This is what you want when you publish to registry, but during development
...
You can, of course, go down `lib/` path, but that requires you to
rebuild your package and transpile it to ES5 whenever you introduce a change,
which can be a pain.

### Building

Building package can be a little bit tricky due to how webpack handles
symlinks. We've found it easiest to remove symlink and replace it with
actual files, either by copying package to `node_modules` or running
`npm install` if you've published your package to npm registry.
