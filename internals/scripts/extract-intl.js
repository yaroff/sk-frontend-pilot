/* eslint-disable */
/**
 * This script will extract internationalization messages from all components
   and package them in translation json files in translations file.
 */
const fs = require('fs');
...
for (const locale of locales) {
  ...
  try {
    // Parse old translation message JSON files
...
/* push `react-intl` plugin to existing plugins that are already configured in `package.json`
   Example:
   ```
  "babel": {
    "plugins": [
  ...
  try {
    ...
    for (const message of result['react-intl'].messages) {
      for (const locale of locales) {
        ...
        // Merge old translations into babel extracted instances where react-intl is used
...
(async function main() {
  ...
  // Run extraction on all files that match glob on line 16
  await Promise.all(files.map((fileName) => extractFromFile(fileName)));
  extractTaskDone()

  // Make directory if it doesn't exist, especially for first run
  ...
  for (const locale of locales) {
    ...
    try {
      ...
      // Sort translation JSON file so that git diffing is easier
      // Otherwise translation messages will jump around every time we extract
      let messages = {};
      Object.keys(localeMappings[locale]).sort().forEach(function(key) {
        messages[key] = localeMappings[locale][key];
      });

      // Write to file JSON representation of translation messages
